/*
 * 多图上传对话框逻辑代码: 远图片/上传图片/在线图片/搜索图片
 */
	...
	RemoteImage.prototype = {
		...
		setImage: function(img) {
			...
			var wordImgFlag = img.getAttribute("word_img"),
				src = wordImgFlag 
					? wordImgFlag.replace("&amp;", "&") 
					: (img.getAttribute('_src') || img.getAttribute("src", 2).replace("&amp;", "&")),align = editor.queryCommandValue("imageFloat");
		...
		setPreview: function() {
			...
			if (url) {
				preview.innerHTML = '<img src="' + url + '" width="' + width + '" height="' + height + '" border="' + border +
	...
	UploadImage.prototype = {
		...
		initUploader: function() {
			...
			uploader.on('uploadBeforeSend', function(file, data, header) {
				header['X_Requested_With'] = 'XMLHttpRequest';//这里可以通过data对象添加POST参数
			});
			...
			uploader.on('error', function(code, file) {
				if (code == 'Q_TYPE_DENIED' || code == 'F_EXCEED_SIZE') addFile(file);
			});
		...
		getInsertList: function() {
			var i, data, list = [],
				align = getAlign(),
				prefix = editor.getOpt('imageUrlPrefix') || $.QD__NET__;
	...
	OnlineImage.prototype = {
		...
		pushData: function(list) {
			var i, item, img, icon, _this = this,
				urlPrefix = editor.getOpt('imageManagerUrlPrefix') || $.QD__NET__;;
			for (i = 0; i < list.length; i++) {
				if (list[i] && list[i].url) {
					...
					img.width = 113;
					img.setAttribute('src', urlPrefix + list[i].url + (list[i].url.indexOf('?') == -1 ? '?noCache=' : '&noCache=') +
	...
	SearchImage.prototype = {
		...
		getImageData: function() {
			var _this = this,
				...
				url = "http://image.baidu.com/i?ct=201326592&cl=2&lm=-1&st=-1&tn=baiduimagejson&istype=2&rn=32&fm=index&pv=&word=" +
				_this.encodeToGb2312(key) + type + "&keeporiginname=" + keepOriginName + "&" + new Date;
			...
			if (0) {
				...
				url = "https://image.baidu.com/search/acjson?tn=resultjson_com&ipn=rj&word=" + key + "&rn=60&nc=" + keepOriginName + "&gsm=" + +new Date;
				ajax.request(url, {
					'dataType': 'jsonp',
					'onsuccess': function(json) {
						var list = [];
